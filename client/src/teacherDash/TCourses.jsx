...
import { Avatar, AvatarFallback } from "@/components/ui/avatar";
import {
  Eye, BarChart3, Trash, BookOpen, Clock, Search, Filter,
  AlertCircle, Copy, Users, Globe, Lock,
} from "lucide-react";
import { useAppContext } from "../context/AppContext.jsx";
import {
...
export default function TCourses() {
  ...
  // ── Filter + search ────────────────────────────────────────────────────────
  // Fixed: was recomputed on every render without memoisation.
  const filteredPaths = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    return (teachersPath ?? []).filter((course) => {
      if (q && !course.title?.toLowerCase().includes(q) && !course.description?.toLowerCase().includes(q)) return false;
      if (filterStatus === "private") return course.isPrivate;
      if (filterStatus === "public")  return !course.isPrivate;
      return true;
    });
  }, [teachersPath, searchQuery, filterStatus]);
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      await axios.delete(`/api/learningpaths/delete/${courseToDelete._id}`);
      setTeachersPath((prev) => prev.filter((c) => c._id !== courseToDelete._id));
      toast.success(`"${courseToDelete.title}" deleted.`);
      setDeleteOpen(false);
      setCourseToDelete(null);
    } catch (err) {
      console.error("[TCourses] delete failed:", err);
      // Fixed: replaced alert() with toast — consistent UX, no browser dialog
      toast.error("Failed to delete the path. Please try again.");
    } finally {
      setIsDeleting(false);
    }
  }, [courseToDelete, isDeleting, axios, setTeachersPath]);

  // ── View ──────────────────────────────────────────────────────────────────
  // Fixed: was searching `paths` (all public paths) but should use teachersPath
  // first, then fall back to paths. Also replaced alert() with toast.
  const handleView = useCallback((courseId) => {
    const found =
      (teachersPath ?? []).find((p) => p._id === courseId) ??
  ...
  return (
    <div className="flex-1 h-[90vh] overflow-y-auto bg-slate-50/50 dark:bg-slate-950 no-scrollbar">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <motion.div variants={itemVariants}>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              {filteredPaths.length} of {teachersPath?.length ?? 0} paths shown
            </p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 md:w-64">
              ...
              <Input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-9 bg-white dark:bg-slate-900 border-slate-200 rounded-xl"
              />
            </div>

            {/* Filter */}
            <Select value={filterStatus} onValueChange={setFilterStatus}>
              <SelectTrigger className="w-[140px] bg-white dark:bg-slate-900 rounded-xl">
                <div className="flex items-center gap-2">
                  <Filter className="h-3.5 w-3.5 text-slate-500" />
                  <SelectValue placeholder="Status" />
                </div>
          ...
          <AnimatePresence mode="popLayout">
            {filteredPaths.length === 0 ? (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="col-span-full text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800"
              >
                <BookOpen className="h-10 w-10 mx-auto text-slate-300 mb-3" />
                <p className="text-muted-foreground font-medium">No learning paths found.</p>
                {searchQuery && (
                  <button
              ...
              filteredPaths.map((course) => (
                <motion.div key={course._id} variants={itemVariants} layout>
                  <Card className="group overflow-hidden border-slate-200 dark:border-slate-800 hover:shadow-xl transition-all duration-300 bg-white dark:bg-slate-900 flex flex-col h-full rounded-2xl border-0 shadow-sm">
                    ...
                    <div className="aspect-video relative overflow-hidden bg-slate-100 dark:bg-slate-800">
                      ...
                      {/* <div className="absolute top-2.5 right-2.5">
                        <Badge className={`text-[10px] gap-1 border-0 shadow ${course.isPrivate ? "bg-slate-800/80 text-white" : "bg-emerald-500/90 text-white"}`}>
                          {course.isPrivate
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                        <Button
                          size="sm"
                          variant="secondary"
                          className="w-full bg-white/90 hover:bg-white text-slate-900 rounded-lg text-xs"
                    ...
                    <CardHeader className="pb-2 pt-3 px-4">
                      <Badge variant="outline" className="text-[10px] uppercase tracking-wider text-slate-500 border-slate-200 w-fit mb-1">
                        {course.category || "General"}
                      </Badge>
                      <CardTitle className="text-base line-clamp-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                        {course.title}
                      </CardTitle>
                    ...
                    {/* Stats */}
                    <CardContent className="pb-3 flex-grow px-4">
                      <div className="grid grid-cols-2 gap-y-1.5 gap-x-3 text-xs text-slate-500 dark:text-slate-400">
                        <div className="flex items-center gap-1.5">
                    ...
                    <CardFooter className="pt-0 pb-4 px-4 flex gap-2">
                      <Button
                        variant="outline"
                        className="flex-1 border-slate-200 hover:bg-slate-50 hover:text-indigo-600 rounded-xl text-xs"
      ...
      <Dialog open={viewOpen} onOpenChange={setViewOpen}>
        <DialogContent className="max-w-4xl p-0 overflow-hidden flex flex-col h-[85vh]">
          ...
          <div className="relative h-48 bg-slate-900 shrink-0">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/50 to-transparent flex flex-col justify-end p-6">
              <div className="flex gap-2 mb-2 flex-wrap">
                <Badge className="bg-indigo-600 hover:bg-indigo-700 border-0">{viewingCourse?.category}</Badge>
                <Badge variant="outline" className="text-white border-white/20 bg-white/10 backdrop-blur-sm">{viewingCourse?.level}</Badge>
              </div>
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <ScrollArea className="flex-1 p-6 bg-slate-50 dark:bg-slate-950">
              <div className="space-y-5">
                {viewingCourse?.description && (
                  <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border shadow-sm">
                    <h3 className="font-semibold text-xs text-slate-500 mb-2 uppercase tracking-wide">About this path</h3>
                    <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed">{viewingCourse.description}</p>
                  </div>
                )}

                <div>
                  <h3 className="font-bold text-base mb-3 flex items-center gap-2">
                    <BookOpen className="h-4 w-4 text-indigo-600" /> Curriculum
                    <span className="text-xs font-normal text-muted-foreground">({sections.length} modules)</span>
                  </h3>
                  <div className="space-y-2.5">
                    {sections.length === 0 ? (
                      ...
                      sections.map((section, idx) => (
                        <div key={section._id ?? idx} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-4 shadow-sm">
                          <div className="flex justify-between items-start mb-1.5">
                            <h4 className="font-semibold text-sm text-slate-900 dark:text-slate-100">
                              {idx + 1}. {section.title}
                            </h4>
                            <span className="text-xs font-mono text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded shrink-0 ml-2">
                              {section.duration || "--:--"}
                            </span>
                          </div>
                          {section.description && (
                            <p className="text-xs text-slate-500 mb-2 line-clamp-2">{section.description}</p>
                          )}
                          <div className="flex flex-wrap gap-1.5">
                            {section.resources?.map((res, rIdx) => (
                              <Badge key={rIdx} variant="secondary" className="font-normal text-[10px] px-2 py-0.5 h-5">
                                {res.fileType ? res.fileType.toUpperCase().slice(0, 3) : "FILE"}
                              </Badge>
                            ))}
                            {section.urls?.filter(Boolean).length > 0 && (
                              <Badge variant="secondary" className="font-normal text-[10px] px-2 py-0.5 h-5">
                                {section.urls.filter(Boolean).length} link{section.urls.filter(Boolean).length > 1 ? "s" : ""}
                              </Badge>
                            )}
                          </div>
                        </div>
            ...
            {/* Sidebar details */}
            <div className="w-72 border-l bg-white dark:bg-slate-900 p-5 shrink-0 flex flex-col gap-5 overflow-y-auto">
              <div>
                <h3 className="font-semibold text-xs text-slate-500 mb-3 uppercase tracking-wide">Instructor</h3>
                <div className="flex items-center gap-3">
                  <Avatar className="h-9 w-9 border">
                    <AvatarFallback className="text-sm">{createdBy.fullName?.[0] ?? "T"}</AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="font-medium text-sm">{createdBy.fullName || "Instructor"}</p>
              ...
              <div className="space-y-2 border-t pt-4">
                {[
                  ["Duration",  viewingCourse?.duration || "N/A"],
                  ["Modules",   `${sections.length} modules`],
                  ["Enrolled",  `${viewingCourse?.learners?.length ?? 0} students`],
                ].map(([label, val]) => (
                  <div key={label} className="flex justify-between items-center py-1.5 border-b border-slate-50 dark:border-slate-800">
                    <span className="text-xs text-slate-500">{label}</span>
                    <span className="font-medium text-xs text-slate-800 dark:text-slate-200">{val}</span>
                  </div>
                ))}
              </div>

              <div className="mt-auto space-y-2.5">
                <Button className="w-full bg-slate-900 hover:bg-indigo-700 text-white rounded-xl transition-colors" onClick={copyLink}>
                  <Copy className="h-4 w-4 mr-2" />
                  {copied ? "Copied!" : "Share Code"}
                </Button>
                <Button variant="outline" className="w-full rounded-xl" onClick={() => setViewOpen(false)}>
                  Close Preview
                </Button>
              </div>
            </div>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="max-w-md w-[90%] rounded-2xl shadow-2xl border-0 p-0 overflow-hidden">
          <div className="bg-red-50 dark:bg-red-900/20 p-6 flex flex-col items-center text-center border-b border-red-100 dark:border-red-800">
            <div className="h-12 w-12 bg-red-100 dark:bg-red-900/40 text-red-600 rounded-full flex items-center justify-center mb-4">
              <AlertCircle className="h-6 w-6" />
            </div>
            <DialogTitle className="text-xl font-bold text-red-900 dark:text-red-200">Delete Learning Path?</DialogTitle>
            <DialogDescription className="text-red-700 dark:text-red-300 mt-1 text-sm">
              This action cannot be undone.
            </DialogDescription>
          </div>
          ...
          <div className="p-6">
            <p className="text-sm text-slate-600 dark:text-slate-300 text-center mb-6">
              ...
              <strong>"{courseToDelete?.title}"</strong>. All student progress associated
              with this path will be lost.
            </p>
            <DialogFooter className="flex gap-3 sm:justify-center">
              <Button
                variant="outline"
